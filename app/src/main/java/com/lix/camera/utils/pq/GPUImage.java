...
public class GPUImage {
    private final String TAG = GPUImage.class.getSimpleName();
    ...
    public Bitmap getBitmapWithFilterApplied(final Bitmap bitmap) {
        ...
        if(!buffer.checkSupport()) {
            Log.i(TAG, "the size of the bitmap is too big for GPU to handle it");
            return bitmap;
        }
        
        buffer.prepareGLEnv();
        buffer.setRenderer(renderer);
        buffer.setPixelBufferListener(new PixelBuffer.PixelBufferListener() {
            @Override
     ...
     * listener.
     * 
     * @param oldFilePath the old file path before GPUImage handle
     * @param newFilePath the new file path GPUImage will save to
     * @param listener the listener
     */
    public void saveToPictures(final String oldFilePath, final String newFilePath,
            final OnPictureSavedListener listener) {
        saveToPictures(mCurrentBitmap, oldFilePath, newFilePath, listener);
    }
    ...
    private class SaveTask extends AsyncTask<Void, Void, Void> {
        ...
        private void saveImage(final String oldFilePath, final String newFilePath, final Bitmap image) {
            File path = Environment
                    .getExternalStoragePublicDirectory(Environment.DIRECTORY_PICTURES);
            File file = new File(path, oldFilePath + "/" + newFilePath);
            try {
                file.getParentFile().mkdirs();
                image.compress(CompressFormat.JPEG, 96, new FileOutputStream(file));
