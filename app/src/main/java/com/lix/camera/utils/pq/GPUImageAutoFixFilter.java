...
public class GPUImageAutoFixFilter extends GPUImageFilter {
    public static final String AUTO_FIX_FRAGMENT_SHADER = 
            ...
            "  vec4 color = texture2D(tex_sampler_0, textureCoordinate);\n" +
    ...
    private int mShiftScaleLocation;
    private int mHistOffsetLocation;
    private int mHistScaleLocation;
    private int mDensityOffsetLocation;
    private int mDensityScaleLocation;
    
    private int mInputHistogramTextureLocation;
    private int mInputDensityTextureLocation;
    
    private float mAutoFixScale;
    private Bitmap mHistogramSourceBitmap;
    private int mBitmapWidth;
    private int mBitmapHeight;
    
    public GPUImageAutoFixFilter(final float scale, final Bitmap bitmap) {
        super(NO_FILTER_VERTEX_SHADER, AUTO_FIX_FRAGMENT_SHADER);
        mAutoFixScale = scale;
        mHistogramSourceBitmap = bitmap;
        mBitmapWidth = mHistogramSourceBitmap.getWidth();
        mBitmapHeight = mHistogramSourceBitmap.getHeight();
    }
    ...
    public void onInit() {
        ...
        mShiftScaleLocation = GLES20.glGetUniformLocation(getProgram(), "shift_scale");
        mHistOffsetLocation = GLES20.glGetUniformLocation(getProgram(), "hist_offset");
        mHistScaleLocation = GLES20.glGetUniformLocation(getProgram(), "hist_scale");
        mDensityOffsetLocation = GLES20.glGetUniformLocation(getProgram(), "density_offset");
        mDensityScaleLocation = GLES20.glGetUniformLocation(getProgram(), "density_scale");
        
        mInputHistogramTextureLocation = GLES20.glGetUniformLocation(getProgram(), "tex_sampler_1");
        mInputDensityTextureLocation = GLES20.glGetUniformLocation(getProgram(), "tex_sampler_2");
    }

    @Override
    public void onInitialized() {
        super.onInitialized();
        setFloat(mScaleLocation, mAutoFixScale);
        setFloat(mShiftScaleLocation, 1.0f / 256f);
        setFloat(mHistOffsetLocation, 0.5f / 766f);
        setFloat(mHistScaleLocation, 765f / 766f);
        setFloat(mDensityOffsetLocation, 0.5f / 1024f);
        setFloat(mDensityScaleLocation, 1023f / 1024f);
    }
    ...
    protected void onDrawArraysPre() {
        ...
        mHistogramSourceBitmap.getPixels(pixels, 0, mBitmapWidth, 0, 0, mBitmapWidth, mBitmapHeight);
        GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, prepareHistogramTexture(mHistogramSourceBitmap.getWidth(), mHistogramSourceBitmap.getHeight(), pixels));
        GLES20.glUniform1i(mInputHistogramTextureLocation, 3);
        
        GLES20.glActiveTexture(GLES20.GL_TEXTURE5);
        GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, prepareDensityTexture());
        GLES20.glUniform1i(mInputDensityTextureLocation, 5);
    }

    public void setAutoFixScale(final float scale) {
        mAutoFixScale = scale;
        setFloat(mScaleLocation, mAutoFixScale);
    }
    
    private int prepareHistogramTexture(int width, int height, int[] data) {
        ...
        IntBuffer histogramData = IntBuffer.wrap(histArray);
        
        return OpenGlUtils.loadTexture(histogramData, histDims, 1, OpenGlUtils.NO_TEXTURE);
    }
    
    private int prepareDensityTexture() {
        int densityDim = 1024;
        int histDim = 255 * 3 + 1;
        ...
        IntBuffer densityData = IntBuffer.wrap(densityTable);
        
        return OpenGlUtils.loadTexture(densityData, densityDim, 1, OpenGlUtils.NO_TEXTURE);
    }
}